...
print("Welcome to the FSMD simulator! - Version 1.0 - Designed by: Jacopo Ceccuti, Andrea Fratini and Erik Sanz")
...
while True:
    ...
    if state == initial_state:
        ...
        execute_operation(operations["init_A"])
        execute_operation(operations["init_TH"])
    
        print('Cycle:', cycle)
        print('Current state:', state)
        for t in fsmd["INITIALIZE"]:
            print("The condition: " + "("+t["condition"]+")" + " is true.")
            print("Executing instruction: " + t["instruction"])
            print("Next state: " + t["nextstate"])
        ...
        state = fsmd["INITIALIZE"][0]["nextstate"]
        if cycle==iterations: 
            break
        cycle += 1
        ...
        #check the differrent conditions and do the respective instuction
        #update state if needed
    
        if evaluate_condition(conditions["A_equal_TH"]):
            print('Cycle:', cycle)
            print('Current state:', state)
            print("The condition: " +"("+fsmd["COMPUTE"][0]["condition"]+")" + " is true.")
            print("Executing instruction: " + fsmd["COMPUTE"][0]["instruction"])
            print("Next state: " + fsmd["COMPUTE"][0]["nextstate"])
            print('At the end of cycle', cycle, 'execution, the status is:')
            print('Variables:')
            for key,value in variables.items():
                print("\t"+str(key)+' = '+str(value)) 
            state = fsmd["COMPUTE"][0]["nextstate"]
            
        elif evaluate_condition(conditions["A_greater_TH"]):
            print('Cycle:', cycle)
            print('Current state:', state)
            print("The condition: " +"("+fsmd["COMPUTE"][1]["condition"]+")" + " is true.")
            print("Executing instruction: " + fsmd["COMPUTE"][1]["instruction"])
            execute_operation(operations["decr_A"])
            print("Next state: " + fsmd["COMPUTE"][1]["nextstate"])
            print('At the end of cycle', cycle, 'execution, the status is:')
            print('Variables:')
            for key,value in variables.items():
                print("\t"+str(key)+' = '+str(value)) 
            state = fsmd["COMPUTE"][1]["nextstate"]
            
        elif evaluate_condition(conditions["TH_greater_A"]):
            print('Cycle:', cycle)
            print('Current state:', state)
            print("The condition: " +"("+fsmd["COMPUTE"][2]["condition"]+")" + " is true.")
            print("Executing instruction: " + fsmd["COMPUTE"][2]["instruction"])
            execute_operation(operations["incr_A"])
            print("Next state: " + fsmd["COMPUTE"][2]["nextstate"])
            print('At the end of cycle', cycle, 'execution, the status is:')
            print('Variables:')
            for key,value in variables.items():
                print("\t"+str(key)+' = '+str(value))     
            state = fsmd["COMPUTE"][2]["nextstate"]
        ...
        print('Cycle:', cycle)
        print('Current state:', state)
        print("The condition: " +"("+fsmd["DONE"][0]["condition"]+")" + " is true.")
        print("Executing instruction: " + fsmd["DONE"][0]["instruction"])
        print("Next state: " + fsmd["DONE"][0]["nextstate"])
        print('At the end of cycle', cycle, 'execution, the status is:')
        print('Variables:')
        for key,value in variables.items():
            print("\t"+str(key)+' = '+str(value))     
        #state = fsmd["DONE"][0]["nextstate"]
        break
